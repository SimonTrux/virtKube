...
# IMAGE="${HOME}/.local/share/libvirt/images/fedora-coreos-41.20241027.3.0-qemu.x86_64.qcow2"
  IMAGE="${HOME}/.local/share/libvirt/images/alpine-rke2.qcow2"
...
# sudo chcon --verbose --type svirt_home_t ${IGNITION_CONFIG}
  ...
  sudo virt-install --connect="qemu:///system" --name="${VM_NAME}" --vcpus="${VCPUS}" --memory="${RAM_MB}" \
       ...
       #  --boot uefi --boot loader=${HOME}/.local/share/libvirt/images/efivars.fd
         #--boot uefi --boot loader=${HOME}/.local/share/libvirt/images/efivars.fd,loader.readonly=yes,loader.type=pflash
